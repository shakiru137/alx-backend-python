...
async def wait_n(n: int, max_delay: int) -> List[float]:
    ...
    This function spawns `n` asynchronous tasks thateach callthe `wait_random`
    ...
    Returns:
        List[float]:A list of floats representing the actual delays experienced
                     by each call to `wait_random`, ordered by the completion
                     time of the tasks.
    ...
    Note:
        The order of the returned delays is based on the order of task
        completion, not the order in which the tasks were started.
    """
